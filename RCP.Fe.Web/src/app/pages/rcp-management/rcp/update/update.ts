import { IUpdateRCP, IViewRCP } from "@/models/rcp/rcp.models";
...
@Component({
    selector: 'app-update-rcp',
    imports: [SharedImports,Select,FileUpload],
    templateUrl: './update.html',
    styleUrl: './update.scss'
})

export class UpdateRCP  extends BaseComponent{
    ...
    data: IViewRCP | undefined;
    rcpId: number = this._config.data?.id || 0;
    ...
    parseAddressData(): void {
        ...
        getRcpData(){
            ...
            this._rcpService.findById(this.rcpId).subscribe({
                next: (res) => {
                    if(this.isResponseSucceed(res,false)) {
                        ...
                        this.form.patchValue({
                            ...
                            name: this.data?.name,
                            city: this.data?.city,
                            soLuongPhongChieu : this.data?.soLuongPhongChieu,
                        });
                        const selectedProvince = this.provinceOptions.find(p => p.value === this.data?.city);
                        if (selectedProvince) {
                            this.districtOptions = this.wards.filter(d => d.province_code === selectedProvince.code)
        ...
        onSubmitUpdate(){
            const body :IUpdateRCP = {
                id:this.rcpId,
                ...this.form.value,
                fileAnhCinema: this.selectedFile
            };
            this.loading = true;
            this._rcpService.update(body).subscribe({
                next:(res) => {
                    if(this.isResponseSucceed(res,true,'Đã cập nhật thông tin rạp chiếu phim thành công')){
